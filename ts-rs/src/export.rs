...
pub(crate) fn export_type<T: TS + 'static + ?Sized>() -> Result<(), ExportError> {
    let path = output_path::<T>()?;
    export_type_to::<T, _>(&path)
}

/// Export `T` to the file specified by the `path` argument.
pub(crate) fn export_type_to<T: TS + 'static + ?Sized, P: AsRef<Path>>(
    path: P,
) -> Result<(), ExportError> {
    #[allow(unused_mut)]
...
pub(crate) fn export_type_to_string<T: TS + 'static + ?Sized>() -> Result<String, ExportError> {
...
fn generate_imports<T: TS + 'static + ?Sized>(out: &mut String) -> Result<(), ExportError> {
